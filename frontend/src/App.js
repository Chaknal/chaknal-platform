...
function AppContent() {
  ...
  const [isAgency, setIsAgency] = useState(true); // Set to true for agency demo
  ...
  useEffect(() => {
    const loadCompanyLogo = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/api/company-settings/branding/${companyId}`);
        const data = await response.json();
        
        if (data.success && data.data.logo_url) {
          setCompanyLogo(`http://localhost:8000${data.data.logo_url}`);
        } else {
          setCompanyLogo(null);
        }
      } catch (error) {
  ...
  // Simple mock user for local development
  const currentUser = {
    email: "agency@chaknal-demo.com",
    company: { name: "Agency Company", domain: "agency.com" },
    organization: { name: "Agency Dashboard" },
    is_agency: true,
    role: "agency"
  };
      ...
      <div className="flex">
          ...
          <div className="p-4">
            <div className="mb-6">
              ...
              <div className="text-sm font-medium text-gray-900">
                {isAgency && currentClient ? currentClient.name : (currentUser.company?.name || 'N/A')}
              </div>
              <div className="text-xs text-gray-500">
                {isAgency && currentClient ? currentClient.domain : (currentUser.company?.domain || '')}
              </div>
            </div>
        ...
        <div className="flex-1 p-8">
          ...
          <Routes>
            ...
            <Route path="/campaigns" element={<CampaignsNew currentClient={currentClient} isAgency={isAgency} />} />
