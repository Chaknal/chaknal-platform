...
const ClientSwitcher = ({ onClientSwitch, currentClient }) => {
  ...
  const fetchClients = async () => {
    try {
      ...
      const response = await fetch('https://chaknal-backend-container.azurewebsites.net/api/agency/clients', {
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success && data.clients) {
          ...
          const transformedClients = data.clients.map(client => ({
            id: client.id,
            name: client.name,
            domain: client.domain || `${client.name.toLowerCase().replace(/\s+/g, '')}.com`,
            access_level: client.access_level || 'full',
            stats: {
              dux_accounts: client.stats?.dux_accounts || 1,
              campaigns: client.stats?.campaigns || 0,
              contacts: client.stats?.contacts || 0,
              messages: client.stats?.messages || 0
            }
          }));
          
          setClients(transformedClients);
          // Set first client as default if none selected
          if (!currentClient && transformedClients.length > 0) {
            onClientSwitch(transformedClients[0]);
          }
          return;
        }
      }
      ...
      console.log('API failed, using mock data for ClientSwitcher');
